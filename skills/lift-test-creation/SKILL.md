...
|impactCampaignInfos|**Either/or**|Tactic ID(s) when testLevel = tactic; campaign ID(s) when testLevel = campaign; account ID(s) when testLevel = platform. Fetch **account / campaign** candidates with lift-test-impact-campaigns; fetch **tactic** candidates with the tactic MCP tool (impact-campaigns does not return tactics).|
...
3. **Resolve defaults.** Run lift-test-scan for: current spend, suggested params, and existing tests for collision avoidance (`runningTests` covers draft / scheduled / executing / active; auto-exclude the live ones, drafts only when the user names them). Query DB for sales channels and country. Apply any stated constraints to the inputs before design. (The PTM / LTM sufficiency call is made later, by lift-test-design-analyze, not by scan.)
...
|---|---|---|
|lift-test-scan|Required|Last-28-day daily spend + orders, existing tests as `runningTests` (draft / scheduled / executing / active) for awareness and start-date conflict checks, and suggested params (holdout / cells / metric). The actual geo auto-exclusion set is computed later by design-prepare (loadConcurrentTestInfo), not by scan.|
