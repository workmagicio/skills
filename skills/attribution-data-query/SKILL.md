...
| "Create / build a dashboard" | build a live-data dashboard **artifact** — the `dashboard` skill (an html/react live view), not a native platform dashboard. If the ask is a **recurring business review** ("weekly overview", "monthly review"), route to `attribution-weekly-report` — it owns the board spec for each cadence |
