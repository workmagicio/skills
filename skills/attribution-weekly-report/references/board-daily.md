...
const PERIOD = {
  ...
  days: 1,
  buckets: 30,
  baseline: "trailing",          // ← the important one
  noun: "day",
  unitPlural: "days",
  adjective: "Daily",
...
So the daily board's comparison baseline is the **trailing 7-day median of the same
metric**, and the KPI deltas are labelled as such. This is the only cadence that sets
`baseline: "trailing"`, and it is the single largest reason a daily board built by copying
the weekly one is wrong.
