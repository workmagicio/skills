...
- **Fabricate campaign-name examples** — always pull real ones from `database-query-run`. Invented examples destroy user trust.
...
- **Skip `database-query-ask` before SQL** — required for `ctx` timestamp on both the sample pull and the sanity check
